...
func setup(gap_center_y: float) -> void:
	...
	top_pipe.position = Vector2(-40, 0)
	top_pipe.size = Vector2(80, top_height)
	top_collision.position = Vector2(40, top_height / 2.0)
	top_collision.shape.size = Vector2(80, top_height)
	
	# 下管道：从间隙下沿到屏幕底部
	var bottom_y = gap_center_y + half_gap
	var bottom_height = 720 - bottom_y
	bottom_pipe.position = Vector2(-40, bottom_y)
	bottom_pipe.size = Vector2(80, bottom_height)
	bottom_collision.position = Vector2(40, bottom_height / 2.0)
